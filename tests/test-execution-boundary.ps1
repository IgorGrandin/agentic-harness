...
$testRoot = [IO.Path]::GetFullPath((Join-Path $repoRoot ('.test-output\execution-boundary-' + [Guid]::NewGuid().ToString('N'))))
...
$pwsh = if ($null -ne (Get-Command pwsh -ErrorAction SilentlyContinue)) { (Get-Command pwsh).Source } else { (Get-Process -Id $PID).Path }
...
function New-ArgumentsFile([string]$Name, [string[]]$Values) {
    $path = Join-Path $testRoot $Name
    [IO.File]::WriteAllText($path, ($Values | ConvertTo-Json -Compress), [Text.UTF8Encoding]::new($false))
    return $path
}
...
$successArgs = New-ArgumentsFile 'success-args.json' @('-NoProfile', '-File', $successScript)
$success = Invoke-Runner -RunnerArguments @('-FilePath', $pwsh, '-ArgumentsFile', $successArgs, '-RuntimeRoot', $runtimeTestRoot, '-Task', 'runner-success', '-Phase', 'gate') -OutputPath (Join-Path $testRoot 'success-result.json')
...
$failureArgs = New-ArgumentsFile 'failure-args.json' @('-NoProfile', '-File', $failureScript)
$failure = Invoke-Runner -RunnerArguments @('-FilePath', $pwsh, '-ArgumentsFile', $failureArgs, '-RuntimeRoot', $runtimeTestRoot, '-TailLines', '5') -OutputPath (Join-Path $testRoot 'failure-result.json')
...
$timeoutArgs = New-ArgumentsFile 'timeout-args.json' @('-NoProfile', '-File', $timeoutScript)
$timeout = Invoke-Runner -RunnerArguments @('-FilePath', $pwsh, '-ArgumentsFile', $timeoutArgs, '-RuntimeRoot', $runtimeTestRoot, '-TimeoutSeconds', '1') -OutputPath (Join-Path $testRoot 'timeout-result.json')
